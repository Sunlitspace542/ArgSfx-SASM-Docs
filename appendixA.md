...
| `STRING <str>([len])=("txt"/<lbl>)`          | (**ArgSfx only**) Define/assign contents to a named or numbered string.<br>Max length is 150 characters.<br>Length does not need to be specified when assigning contents, but must be specified when a string is initially defined. |
